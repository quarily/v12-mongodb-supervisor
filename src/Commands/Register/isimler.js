...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    if(!member) return message.lineReply(embed.setDescription(`Hangi kullanıcının isim geçmişine bakacağını belirtmelisin \`${ayar.prefix}isimler @Brita/ID\``))

    let isimsorgu = await datas.findOne({guildID: message.guild.id, userID: member.user.id})
    if(!isimsorgu) { return message.lineReply(embed.setDescription(`Belirttiğin kullanıcının her hangi bir isim geçmişi bulunmuyor.`))}
