...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    // return message.lineReply(embed.setDescription(`Bu komutu kullanmak için \`${message.guild.roles.cache.find(x => x.id === conf.Register.RegisterHammer).name}\` yetkisine sahip olman lazım`))
    ...
    if(times< 604800000) return message.lineReply(embed.setDescription(`7 gün önce açılmış hesapları kayıt edemzsin`)) // Bu kısmı Shi'den çaldım <31  shi⁴²#4730 
    if(message.author.id === member.id) { return message.lineReply(embed.setDescription(`Kendini kayıt edemezsin`)) }
    if(message.member.roles.highest.position <= member.roles.highest.position) { return message.lineReply(embed.setDescription(`Senden yüksekte olan birisini kayıt edemezsin.`)) }
    if(conf.Register.BoyRoles.some(x => member.roles.cache.has(x)) && conf.Register.GirlRoles.some(x => member.roles.cache.has(x))) { return message.lineReply(embed.setDescription(`Belirlediğin kullanıcı zaten kayıtlı.`)) }

    let name = args[1];
    let age = args[2];
    if(!name || !age || isNaN(age)) return message.lineReply(embed.setDescription(`Doğru Kullanım Şekli \`${conf.prefix}kayıt @Brita/ID <İsim> <Yaş>`))       
    if(13 >= age) return message.lineReply(embed.setDescription(`13 yaş ve altını kayıt edemezsin.`))
