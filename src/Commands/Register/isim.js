...
run: async (client, message, args, embed, prefix) => {
    ...
    if(!member) return message.lineReply(embed.setDescription(`Hatalı , kullanıcı belirtmeyi unuttun. Doğru kullanım \`${ayar.prefix}isim @Brita/ID <Nick>\``))

    let name = args[1];
    let age = args[2];
    if(!name || !age || isNaN(age)) return message.lineReply(embed.setDescription(`Hatalı , isim değiştirmek için her hangi bir isim ve yaş yazman lazım. Doğru kullanım \`${ayar.prefix}isim @Brita/ID <Nick>\``))       
