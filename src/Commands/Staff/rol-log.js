...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    if (!data) { return message.lineReply(embed.setDescription(`${member} üyesinin rol geçmişi bulunmuyor!`)) }
    ...
    for (var i = 0; i < Math.floor(data.length / 2000); i++) {
      message.lineReply(embed.setDescription(data.slice(0, 2000)));
      data = data.slice(2000);
    }
    message.react(emoji.TikID)
    if (data.length > 0) message.lineReplyNoMention(embed.setDescription(data));
  },
};