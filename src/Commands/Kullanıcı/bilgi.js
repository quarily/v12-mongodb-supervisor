...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    //return message.lineReply(embed.setDescription(`Bu komutu kullanmak için \`${message.guild.roles.cache.find(x => x.id === conf.Staff).name}\` yetkisine sahip olman lazım`))
        ...
        return message.lineReply(embed.setDescription(`Hatalı , rol belirtmeyi unuttun. Doğru kullanım \`${ayar.prefix}@Rol/ID\``))
    }
        
        if(rol.members.size < 1) {
        return message.lineReply(embed.setDescription(`Belirtilen rolde hiç üye bulunmamakta.`))
    }
