...
run: async (client, message, args, embed, prefix) => {
  ...
  if(message.member.roles.cache.has(conf.Staff)) {
    message.lineReply(embed.setDescription(`Yetkili olan kişiler booster komutunu kullanamaz.`))
  }
  if (!message.member.roles.cache.has(conf.Staff) && message.member.roles.cache.has(conf.Booster)){
    let member = message.mentions.members.first() || message.guild.members.cache.get(args[0]);
      ...
      if(!username) return message.lineReply(embed.setDescription(`Hatalı , kullanıcı adı belirtmeyi unuttun. Doğru kullanım \`${ayar.prefix}booster <nick>\``))
      if(username.toLowerCase().includes('.gg') || username.toLowerCase().includes('.com') || username.toLowerCase().includes('discord'))
      return message.lineReply(embed.setDescription(`Belirttiğin kullanıcı adında reklam var bunu isim yapamazsın.`))
      ...
      return message.lineReplyNoMention(embed.setDescription(`Başarıyla ismin \`${Belirlenenİsim}\` olarak değiştirildi.`))
    } 
    if(!message.member.roles.cache.has(conf.Staff)) {
      message.lineReply(embed.setDescription(`Bu komutu kullanmak için sunucumuza boost basmış olman gerekir.`))
    }
}}