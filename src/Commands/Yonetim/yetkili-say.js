...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    let x = args[0];
    if(!x) return message.lineReply(embed.setDescription(`Bir işlem belirtiniz. \`${ayar.prefix}yetkili online / offline\` `)).catch(e => { })
    ...
    if(novoice < 1 ) return message.lineReply(embed.setDescription(`Yetkililerin Hepsi Seste.`))
