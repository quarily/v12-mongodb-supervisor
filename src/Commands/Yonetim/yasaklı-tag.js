...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    let x = args[0];
    if(!x) return message.lineReply(embed.setDescription(`Hatalı , işlem belirtmelisiniz. Doğru Kullanım \`${ayar.prefix}ytag <ekle / kaldır / liste>\` `)).catch(e => { })
    ...
    if(x == "ekle") {
        ...
        if(!tag || !reason) return message.lineReply(embed.setDescription(`Hatalı , işlem belirtmelisiniz. Doğru Kullanım \`${ayar.prefix}ytag ekle <Tag / Sebep>\``))
        ...
        xyz.save().then(x => {
            ...
            message.lineReplyNoMention(embed.setDescription(`
    ...
    if(x == "liste") {
        ...
        Tagdaki Toplam Kişi: \`${message.guild.members.cache.filter(m => m.user.username.includes(x.Tag)).size}\`
            `).join("\n**────────────**\n") : `Sunucuda belirlenen herhangi bir yasaklı tag yok.`;
                message.lineReply(embed.setAuthor(`${message.guild.name}`, message.author.avatarURL({ dynamic: true })).setDescription(`${btagp}`))
    }

    if(x == "kaldır") {
        let tag = args[1]
        if (!tag) return message.lineReply(embed.setDescription(`Geçerli bir tag belirtmelisin.`))
